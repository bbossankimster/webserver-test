...
def get_wtest_obj(mocker):
    def _get_wtest_obj(return_mocked=False):
        mocked_send_request = mocker.patch.object(send_req, "send_request_async", return_value=["200", "STUB_OUT", None, 1])
...
def test_result(get_wtest_obj):
    wtest = get_wtest_obj(return_mocked=False)
    # 1) txt
    assert sorted([*wtest.txt_summary]) == ['all', 'no_errors']
    ...
    # 2) dataframes
    assert wtest.errors.empty == True
    assert wtest.no_errors.empty == False
...
def test_analyze(get_wtest_obj):
    ...
    assert after_analyze == "http тест завершен. Ошибок не найдено!"